...
async function runCreateNodeAgent({ prompt, seedGraph, sourceId, sourceHandle = null, flowId = null, history = [], send, done }) {
  try {
      ...
      func: async (input) => {
        try {
          ...
          const forwardSend = (obj) => {
            try {
              // Propager le flux du sous-agent avec un méta champ 'agent'
              const basePath = ['nodeargs'];
              if (obj?.type === 'tool.start') { send({ type: 'tool.start', name: obj.name, args: obj.args, agent: 'nodeargs', agentPath: basePath }); return; }
              if (obj?.type === 'tool.end') { send({ type: 'tool.end', name: obj.name, ok: obj.ok, agent: 'nodeargs', agentPath: basePath }); return; }
              if (obj?.type === 'done') { /* fin de sous-agent: pas d'événement synthétique */ return; }
              if (obj?.type === 'await_user') { awaitUserInput = true; send({ type: 'await_user', question: obj?.question || '', agent: 'nodeargs', agentPath: basePath }); return; }
              if (obj?.type === 'args.partial') { send({ type: 'args.partial', args: obj.args, agent: 'nodeargs', agentPath: basePath }); return; }
              else if (obj?.type === 'args') { pendingArgs = obj.args || {}; argsLocked = true; send({ type: 'args', args: obj.args, agent: 'nodeargs', agentPath: basePath }); }
                ...
                send({ type: 'desc', text: pendingDesc, agent: 'nodeargs', agentPath: basePath });
              }
              else if (obj?.type) send({ ...obj, agent: 'nodeargs', agentPath: basePath });
            } catch { }
          };
    ...
    const model = new ChatOpenAI({ temperature: 1, modelName: process.env.OPENAI_MODEL || 'gpt-5', openAIApiKey: process.env.OPENAI_API_KEY || process.env.OPENAI_KEY || process.env.OPENAI_APIKEY || '', streaming: true });
      ...
      try {
        ...
        for await (const ev of stream) {
          try {
            ...
            else if (ev.event === 'on_tool_start') { send({ type: 'tool.start', name: ev.name, args: ev.data?.input?.input || {}, agent: 'nodeargs' }); }
