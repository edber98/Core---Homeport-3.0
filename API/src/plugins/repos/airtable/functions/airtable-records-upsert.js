...
module.exports = {
  async airtable_records_upsert(node, msg, inputs, opts) {
    const d = inputs || {};
    ...
    try { fieldsToMergeOn = parseJson(d.fieldsToMergeOn, "fieldsToMergeOn"); } catch (e) { return { ok: false, error: e.message }; }
    if (!Array.isArray(records) || !records.length) return { ok: false, error: "records must be a non-empty JSON array." };
    if (!Array.isArray(fieldsToMergeOn) || !fieldsToMergeOn.length) return { ok: false, error: "fieldsToMergeOn must be a non-empty JSON array." };

    const res = await utils.airtableRequest(opts, `/${encodeURIComponent(baseId)}/${encodeURIComponent(tableIdOrName)}`, {
      method: "PATCH",
      body: { records, performUpsert: { fieldsToMergeOn } }
    });
