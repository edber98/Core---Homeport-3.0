...
module.exports = {
  async github_commits_list(node, msg, inputs, opts) {
    ...
    const commits = (Array.isArray(res.data) ? res.data : []).map(r => ({ sha: r.sha, message: r.commit?.message, author: r.commit?.author?.name, date: r.commit?.author?.date, html_url: r.html_url }));
