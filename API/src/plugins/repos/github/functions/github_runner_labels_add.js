...
module.exports = {
  async github_runner_labels_add(node, msg, inputs, opts) {
    const d = inputs || {};
    ...
    if (!labels.length) return { ok: false, error: "labels requis." };
    const res = await utils.githubRequest(opts, `/repos/${owner}/${repo}/actions/runners/${runner_id}/labels`, { method: "POST", body: { labels } });
    if (!res.ok) return res;
    const data = res.data || {};
    return { ok: true, totalCount: data.total_count || labels.length, labels: mapLabels(data) };
  }
};
