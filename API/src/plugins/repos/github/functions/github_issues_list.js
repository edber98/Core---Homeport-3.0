...
module.exports = {
  async github_issues_list(node, msg, inputs, opts) {
    ...
    const issues = (Array.isArray(res.data) ? res.data : []).map(r => ({ id: r.id, number: r.number, title: r.title, state: r.state, html_url: r.html_url, user: r.user?.login, labels: (r.labels || []).map(l => l.name).join(", "), assignees: (r.assignees || []).map(a => a.login).join(", "), created_at: r.created_at }));
