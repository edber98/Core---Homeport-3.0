...
module.exports = {
  async browserbase_session_create(node, msg, inputs, opts) {
    ...
    const json = {};
    try {
      for (const key of ["userMetadata"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/v1/sessions`;
    const query = {};
    const body = ({ projectId: d.projectId, region: d.region, keepAlive: d.keepAlive === 'true' || d.keepAlive === true, userMetadata: json.userMetadata });
