...
module.exports = {
  async huggingface_feature_extraction(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["modelId","inputs"]) {
      if (d[key] === undefined || d[key] === null || d[key] === "") return { ok: false, error: `${key} requis.` };
    }
    const json = {};
    try {
      for (const key of ["inputs","parameters"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/hf-inference/models/${encodeURIComponent(String(d.modelId))}`;
    const query = {};
    const body = ({ inputs: json.inputs !== undefined ? json.inputs : d.inputs, parameters: json.parameters });
