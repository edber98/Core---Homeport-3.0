...
module.exports = {
  async huggingface_text_to_image(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["modelId","prompt"]) {
      if (d[key] === undefined || d[key] === null || d[key] === "") return { ok: false, error: `${key} requis.` };
    }
    const json = {};
    try {
      for (const key of ["parameters"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/hf-inference/models/${encodeURIComponent(String(d.modelId))}`;
    const query = {};
    const body = ({ inputs: String(d.prompt), parameters: json.parameters });
