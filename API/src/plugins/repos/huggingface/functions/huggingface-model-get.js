...
module.exports = {
  async huggingface_model_get(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["modelId"]) {
    ...
    const path = `https://huggingface.co/api/models/${encodeURIComponent(String(d.modelId))}`;
    const query = {};
    const body = {};
