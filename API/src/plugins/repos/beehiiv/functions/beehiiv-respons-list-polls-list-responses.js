...
module.exports = {
  async beehiiv_respons_list_polls_list_responses(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/publications/{publicationId}/polls/{pollId}/responses";
    ...
    if (!pollid) return { ok: false, error: 'pollid requis.' };
    reqPath = reqPath.replace('{pollid}', encodeURIComponent(pollid));
