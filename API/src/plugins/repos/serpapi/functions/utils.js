async function serpapiRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "https://serpapi.com").replace(/\/+$/, "");
...
module.exports = { utils: { serpapiRequest, parseJsonInput, compactJson, firstTextFromChoices } };
