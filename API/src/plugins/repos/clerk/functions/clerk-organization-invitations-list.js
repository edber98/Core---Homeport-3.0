...
module.exports = {
  async clerk_organization_invitations_list(node, msg, inputs, opts) {
    const d = inputs || {};
    const organizationId = String(d.organizationId || "").trim();
    if (!organizationId) return { ok: false, error: "ID organisation requis." };
    const res = await utils.clerkRequest(opts, `/organizations/${encodeURIComponent(organizationId)}/invitations`, {
      query: { limit: utils.toInt(d.pageSize, 50), offset: utils.toInt(d.offset, 0), status: d.status }
    });
