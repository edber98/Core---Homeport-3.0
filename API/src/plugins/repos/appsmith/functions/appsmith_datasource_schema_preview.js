...
module.exports = {
  async appsmith_datasource_schema_preview(node, msg, inputs, opts) {
    const d = inputs || {};
    const query = {};
...
const options = {};
    ...
    const res = await utils.appsmithRequest(opts, `/api/v1/datasources/${encodeURIComponent(String(d.datasourceId || "").trim())}/schema-preview`, { method: 'POST', query, ...options });
