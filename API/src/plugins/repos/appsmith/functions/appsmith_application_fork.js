...
module.exports = {
  async appsmith_application_fork(node, msg, inputs, opts) {
    const d = inputs || {};
    const query = {};
...
const options = {};
    const res = await utils.appsmithRequest(opts, `/api/v1/applications/${encodeURIComponent(String(d.branchedApplicationId || "").trim())}/fork/${encodeURIComponent(String(d.workspaceId || "").trim())}`, { method: 'POST', query, ...options });
