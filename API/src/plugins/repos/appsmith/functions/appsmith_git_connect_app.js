...
module.exports = {
  async appsmith_git_connect_app(node, msg, inputs, opts) {
    const d = inputs || {};
    const query = {};
...
const options = {};
    ...
    const res = await utils.appsmithRequest(opts, `/api/v1/git/connect/app/${encodeURIComponent(String(d.baseApplicationId || "").trim())}`, { method: 'POST', query, ...options });
