...
module.exports = {
  async attio_download_list_download_a_file(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/v2/files/{file_id}/download";
    const file_id = String(d.file_id || '').trim();
    if (!file_id) return { ok: false, error: 'file_id requis.' };
    reqPath = reqPath.replace('{file_id}', encodeURIComponent(file_id));
