...
module.exports = { async miro_board_item_update(node, msg, inputs, opts) {
  ...
  let data, position;
  try { data = utils.parseJsonInput(d.itemData, "itemData", undefined); position = utils.parseJsonInput(d.position, "position", undefined); } catch (e) { return { ok: false, error: e.message }; }
