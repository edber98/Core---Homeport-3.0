...
module.exports = {
  async miro_board_comment_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["boardId","text"]) {
      if (d[key] === undefined || d[key] === null || d[key] === "") return { ok: false, error: `${key} requis.` };
    }
    const json = {};
    try {
      for (const key of ["position"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/boards/${encodeURIComponent(String(d.boardId))}/comments`;
    const query = {};
    const body = ({ data: { text: String(d.text) }, position: json.position });
