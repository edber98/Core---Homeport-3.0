...
module.exports = {
  async miro_board_item_delete(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["boardId","itemId"]) {
    ...
    const path = `/boards/${encodeURIComponent(String(d.boardId))}/items/${encodeURIComponent(String(d.itemId))}`;
    const query = {};
    const body = {};
