...
module.exports = {
  async miro_board_comments_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["boardId"]) {
    ...
    const path = `/boards/${encodeURIComponent(String(d.boardId))}/comments`;
    const query = ({ limit: d.pageSize, cursor: d.cursor });
    const body = {};
    ...
    if (true) {
      const rawItems = utils.asArray(utils.getPath(res.data, "data"));
      const items = rawItems.map(utils.itemFromUnknown);
      return { ok: true, items, totalCount: Number(res.data?.pagination?.total || res.data?.total || res.data?.totalCount || items.length), nextCursor: res.data?.cursor || "" };
    }
