...
module.exports = {
  async miro_board_create(node, msg, inputs, opts) {
    ...
    const json = {};
    try {
      for (const key of ["policy"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/boards`;
    const query = {};
    const body = ({ name: String(d.name), description: d.description, policy: json.policy });
