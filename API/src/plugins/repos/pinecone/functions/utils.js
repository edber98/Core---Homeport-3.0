async function pineconeRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "").replace(/\/+$/, "");
  if (!baseUrl) return { ok: false, error: "Hôte d'index Pinecone manquant." };
...
module.exports = { utils: { pineconeRequest, parseJsonInput, compactJson, firstTextFromChoices } };
