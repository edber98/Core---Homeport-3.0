...
module.exports = {
  async qb_vendor_update(node, msg, inputs, opts) {
    const d = inputs || {};
    const vendorId = (d.vendorId || "").toString().trim();
    if (!vendorId) return { ok: false, error: "Missing vendorId." };

    const getRes = await utils.qbRequest(opts, `/vendor/${encodeURIComponent(vendorId)}`);
    if (!getRes.ok) return { ok: false, error: getRes.error, status: getRes.status, details: getRes.details };

    const existing = (getRes.data && getRes.data.Vendor) || getRes.data || {};
    const body = { Id: vendorId, SyncToken: existing.SyncToken, sparse: true };
    ...
    const res = await utils.qbRequest(opts, "/vendor", { method: "POST", body });
