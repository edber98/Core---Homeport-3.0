...
module.exports = {
  async dolibarr_post_invoices_by_id_usediscount_by_discountid(node, msg, inputs, opts) {
    ...
    const path = `/invoices/${encodeURIComponent(d.id)}/usediscount/${encodeURIComponent(d.discountid)}` + (query ? ("?" + query) : "");
