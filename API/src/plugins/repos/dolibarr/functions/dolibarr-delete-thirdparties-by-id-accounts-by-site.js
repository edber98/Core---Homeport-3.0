...
module.exports = {
  async dolibarr_delete_thirdparties_by_id_accounts_by_site(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    if (!d.site && d.site !== 0) return { ok: false, error: "Champ site requis." };
    const query = d.query || "";
    const path = `/thirdparties/${encodeURIComponent(d.id)}/accounts/${encodeURIComponent(d.site)}` + (query ? ("?" + query) : "");
