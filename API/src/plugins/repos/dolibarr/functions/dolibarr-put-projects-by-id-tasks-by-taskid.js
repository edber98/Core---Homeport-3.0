...
module.exports = {
  async dolibarr_put_projects_by_id_tasks_by_taskid(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    if (!d.taskid && d.taskid !== 0) return { ok: false, error: "Champ taskid requis." };
    const query = d.query || "";
    let body = undefined;
    if (d.body_json) {
      try { body = JSON.parse(d.body_json); } catch (e) { return { ok: false, error: "body_json invalide (JSON attendu)." }; }
    }
    const path = `/projects/${encodeURIComponent(d.id)}/tasks/${encodeURIComponent(d.taskid)}` + (query ? ("?" + query) : "");
