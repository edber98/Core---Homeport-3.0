const { utils } = require("./utils");

module.exports = {
  async dolibarr_get_categories_object_by_type_by_id(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.type && d.type !== 0) return { ok: false, error: "Champ type requis." };
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    const query = d.query || "";
    const path = `/categories/object/${encodeURIComponent(d.type)}/${encodeURIComponent(d.id)}` + (query ? ("?" + query) : "");
    log('Requête API Dolibarr...');
    const res = await utils.dolibarrRequest(opts, path, { method: "GET" });
    if (!res.ok) return { ok: false, error: res.error, status: res.status, details: res.details };
    return { ok: true, data: res.data, totalRecords: res.totalRecords };
  }
};
