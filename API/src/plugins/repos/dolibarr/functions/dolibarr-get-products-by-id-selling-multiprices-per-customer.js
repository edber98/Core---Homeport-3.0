...
module.exports = {
  async dolibarr_get_products_by_id_selling_multiprices_per_customer(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    const query = d.query || "";
    const path = `/products/${encodeURIComponent(d.id)}/selling_multiprices/per_customer` + (query ? ("?" + query) : "");
