...
module.exports = {
  async dolibarr_delete_invoices_by_id_contact_by_contactid_by_type(node, msg, inputs, opts) {
    ...
    const query = d.query || "";
    const path = `/invoices/${encodeURIComponent(d.id)}/contact/${encodeURIComponent(d.contactid)}/${encodeURIComponent(d.type)}` + (query ? ("?" + query) : "");
