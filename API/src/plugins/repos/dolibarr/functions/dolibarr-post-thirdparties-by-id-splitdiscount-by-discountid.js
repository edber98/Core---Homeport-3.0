...
module.exports = {
  async dolibarr_post_thirdparties_by_id_splitdiscount_by_discountid(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    if (!d.discountid && d.discountid !== 0) return { ok: false, error: "Champ discountid requis." };
    const query = d.query || "";
    let body = undefined;
    if (d.body_json) {
      try { body = JSON.parse(d.body_json); } catch (e) { return { ok: false, error: "body_json invalide (JSON attendu)." }; }
    }
    const path = `/thirdparties/${encodeURIComponent(d.id)}/splitdiscount/${encodeURIComponent(d.discountid)}` + (query ? ("?" + query) : "");
