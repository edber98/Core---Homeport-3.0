...
module.exports = {
  async dolibarr_delete_knowledgemanagement_knowledgerecords_by_id(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    const query = d.query || "";
    const path = `/knowledgemanagement/knowledgerecords/${encodeURIComponent(d.id)}` + (query ? ("?" + query) : "");
