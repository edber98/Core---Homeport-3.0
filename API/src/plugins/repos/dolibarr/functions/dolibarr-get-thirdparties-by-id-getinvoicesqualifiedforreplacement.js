...
module.exports = {
  async dolibarr_get_thirdparties_by_id_getinvoicesqualifiedforreplacement(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    const query = d.query || "";
    const path = `/thirdparties/${encodeURIComponent(d.id)}/getinvoicesqualifiedforreplacement` + (query ? ("?" + query) : "");
