...
module.exports = {
  async dolibarr_delete_orders_by_id_contact_by_contactid_by_type(node, msg, inputs, opts) {
    ...
    const query = d.query || "";
    const path = `/orders/${encodeURIComponent(d.id)}/contact/${encodeURIComponent(d.contactid)}/${encodeURIComponent(d.type)}` + (query ? ("?" + query) : "");
