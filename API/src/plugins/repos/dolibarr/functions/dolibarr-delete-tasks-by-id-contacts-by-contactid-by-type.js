...
module.exports = {
  async dolibarr_delete_tasks_by_id_contacts_by_contactid_by_type(node, msg, inputs, opts) {
    ...
    const query = d.query || "";
    const path = `/tasks/${encodeURIComponent(d.id)}/contacts/${encodeURIComponent(d.contactid)}/${encodeURIComponent(d.type)}` + (query ? ("?" + query) : "");
