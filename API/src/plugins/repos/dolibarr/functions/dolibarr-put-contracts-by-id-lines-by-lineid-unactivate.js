...
module.exports = {
  async dolibarr_put_contracts_by_id_lines_by_lineid_unactivate(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    if (!d.id && d.id !== 0) return { ok: false, error: "Champ id requis." };
    if (!d.lineid && d.lineid !== 0) return { ok: false, error: "Champ lineid requis." };
    const query = d.query || "";
    let body = undefined;
    if (d.body_json) {
      try { body = JSON.parse(d.body_json); } catch (e) { return { ok: false, error: "body_json invalide (JSON attendu)." }; }
    }
    const path = `/contracts/${encodeURIComponent(d.id)}/lines/${encodeURIComponent(d.lineid)}/unactivate` + (query ? ("?" + query) : "");
