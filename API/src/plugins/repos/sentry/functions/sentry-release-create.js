...
module.exports = {
  async sentry_release_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["organizationSlug","version"]) {
      if (d[key] === undefined || d[key] === null || d[key] === "") return { ok: false, error: `${key} requis.` };
    }
    const json = {};
    try {
      for (const key of ["projects","refs"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/organizations/${encodeURIComponent(String(d.organizationSlug))}/releases/`;
    const query = {};
    const body = ({ version: String(d.version), projects: json.projects, refs: json.refs });
