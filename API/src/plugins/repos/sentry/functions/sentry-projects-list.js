...
module.exports = {
  async sentry_projects_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["organizationSlug"]) {
    ...
    const path = `/organizations/${encodeURIComponent(String(d.organizationSlug))}/projects/`;
    const query = ({ per_page: d.pageSize, cursor: d.cursor });
    const body = {};
