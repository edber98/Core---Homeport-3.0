...
module.exports = {
  async sentry_project_event_get(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["organizationSlug","projectSlug","eventId"]) {
    ...
    const path = `/projects/${encodeURIComponent(String(d.organizationSlug))}/${encodeURIComponent(String(d.projectSlug))}/events/${encodeURIComponent(String(d.eventId))}/`;
    const query = {};
    const body = {};
