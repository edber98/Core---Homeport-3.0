async function deepseekRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "https://api.deepseek.com").replace(/\/+$/, "");
...
module.exports = { utils: { deepseekRequest, parseJsonInput, compactJson, firstTextFromChoices } };
