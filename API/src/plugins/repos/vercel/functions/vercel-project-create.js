...
module.exports = {
  async vercel_project_create(node, msg, inputs, opts) {
    ...
    const json = {};
    try {
      for (const key of ["gitRepository"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/v9/projects`;
    const query = ({ teamId: d.teamId });
    const body = ({ name: String(d.name), framework: d.framework, gitRepository: json.gitRepository });
