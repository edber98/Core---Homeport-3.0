...
module.exports = {
  async vercel_deployment_cancel(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["deploymentId"]) {
    ...
    const path = `/v12/deployments/${encodeURIComponent(String(d.deploymentId))}/cancel`;
