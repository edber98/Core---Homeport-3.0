...
module.exports = {
  async vercel_deployments_list(node, msg, inputs, opts) {
    ...
    const query = ({ projectId: d.projectId, state: d.state, teamId: d.teamId, limit: d.pageSize, since: d.cursor });
    const body = {};
    ...
    if (true) {
      const rawItems = utils.asArray(utils.getPath(res.data, "deployments"));
      const items = rawItems.map(utils.itemFromUnknown);
      return { ok: true, items, totalCount: Number(res.data?.pagination?.total || res.data?.total || res.data?.totalCount || items.length), nextCursor: res.data?.pagination?.next || "" };
    }
