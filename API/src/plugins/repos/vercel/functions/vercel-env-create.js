...
module.exports = {
  async vercel_env_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["projectId","key","value","target"]) {
    ...
    const path = `/v10/projects/${encodeURIComponent(String(d.projectId))}/env`;
    const query = ({ teamId: d.teamId });
    const body = ({ key: String(d.key), value: String(d.value), target: String(d.target).split(',').map(s => s.trim()).filter(Boolean), type: d.type || 'encrypted' });
