...
module.exports = {
  async odoo_sale_order_lines_list(node, msg, inputs, opts) {
    const data = inputs || {};
    ...
    const res = await utils.odooCall(opts, "sale.order.line", "search_read", [], { domain, limit });
    if (!res.ok) return res;
    const countRes = await utils.odooCall(opts, "sale.order.line", "search_count", [], { domain });
