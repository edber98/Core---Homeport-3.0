...
module.exports = {
  async crisp_messages_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["websiteId","sessionId"]) {
    ...
    const path = `/website/${encodeURIComponent(String(d.websiteId))}/conversation/${encodeURIComponent(String(d.sessionId))}/messages`;
    const query = ({ per_page: d.pageSize, page: d.cursor });
    const body = {};
