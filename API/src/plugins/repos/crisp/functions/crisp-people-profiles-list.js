...
module.exports = {
  async crisp_people_profiles_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["websiteId"]) {
    ...
    const path = `/website/${encodeURIComponent(String(d.websiteId))}/people/profiles`;
    const query = ({ search: d.search, per_page: d.pageSize, page: d.cursor });
    const body = {};
