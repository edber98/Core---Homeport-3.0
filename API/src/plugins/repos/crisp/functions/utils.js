async function crispRequest(opts, requestPath, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  if (true && !credentials.identifier) return { ok: false, error: "Identifiant Crisp manquant." };
  const baseUrl = String(credentials.baseUrl || "https://api.crisp.chat/v1").replace(/\/+$/, "");
  ...
  const headers = {
    "Authorization": `Basic ${Buffer.from(`${credentials.identifier || ""}:${apiKey}`).toString("base64")}`,
    "X-Crisp-Tier": credentials.tier || "plugin",
...
module.exports = { utils: { crispRequest, parseJsonInput, compactJson, getPath, asArray, itemFromUnknown } };
