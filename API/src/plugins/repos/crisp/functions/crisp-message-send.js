...
module.exports = {
  async crisp_message_send(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["websiteId","sessionId","content"]) {
    ...
    const path = `/website/${encodeURIComponent(String(d.websiteId))}/conversation/${encodeURIComponent(String(d.sessionId))}/message`;
    const query = {};
    const body = ({ type: d.type || 'text', from: d.from || 'operator', origin: 'chat', content: String(d.content) });
