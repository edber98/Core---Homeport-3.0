async function qdrantRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "https://api.qdrant.tech:6333").replace(/\/+$/, "");
...
module.exports = { utils: { qdrantRequest, parseJsonInput, compactJson, firstTextFromChoices } };
