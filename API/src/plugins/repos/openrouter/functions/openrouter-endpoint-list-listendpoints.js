...
module.exports = {
  async openrouter_endpoint_list_listendpoints(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/models/{author}/{slug}/endpoints";
    const author = String(d.author || '').trim();
    if (!author) return { ok: false, error: 'author requis.' };
    reqPath = reqPath.replace('{author}', encodeURIComponent(author));
    const slug = String(d.slug || '').trim();
    if (!slug) return { ok: false, error: 'slug requis.' };
    reqPath = reqPath.replace('{slug}', encodeURIComponent(slug));
