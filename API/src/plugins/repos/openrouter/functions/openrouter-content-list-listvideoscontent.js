...
module.exports = {
  async openrouter_content_list_listvideoscontent(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/videos/{jobId}/content";
    const jobid = String(d.jobid || '').trim();
    if (!jobid) return { ok: false, error: 'jobid requis.' };
    reqPath = reqPath.replace('{jobid}', encodeURIComponent(jobid));
