...
module.exports = {
  async webflow_form_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = '/sites/{site_id}/forms';
    ...
    const rawItems = Array.isArray(payload.forms)
      ? payload.forms
    ...
    const items = rawItems.map((r) => ({
      ...
      url: r && (r.formElementId || r.url || r.html_url || ''),
