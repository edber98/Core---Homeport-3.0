...
module.exports = {
  async webflow_asset_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = '/sites/{site_id}/assets';
    ...
    if (d.localeId !== undefined && d.localeId !== null && d.localeId !== '') query.localeId = d.localeId;
    ...
    const rawItems = Array.isArray(payload.assets)
      ? payload.assets
    ...
    const items = rawItems.map((r) => ({
      ...
      name: r && (r.displayName || r.originalFileName || r.name || r.title || ''),
      url: r && (r.hostedUrl || r.url || r.html_url || ''),
