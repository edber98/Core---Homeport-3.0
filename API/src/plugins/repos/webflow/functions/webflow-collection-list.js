...
module.exports = {
  async webflow_collection_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/sites/{site_id}/collections";
    ...
    const rawItems = Array.isArray(payload.collections) ? payload.collections : Array.isArray(payload.items) ? payload.items : Array.isArray(payload.results) ? payload.results : Array.isArray(payload) ? payload : [];
    const items = rawItems.map((r) => ({
      id: r && (r.id || r.uuid || r.key || ''),
      name: r && (r.displayName || r.singularName || r.name || r.title || ''),
