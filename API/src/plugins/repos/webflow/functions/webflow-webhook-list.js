...
module.exports = {
  async webflow_webhook_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/sites/{site_id}/webhooks";
    ...
    const rawItems = Array.isArray(payload.webhooks) ? payload.webhooks : Array.isArray(payload.items) ? payload.items : Array.isArray(payload.results) ? payload.results : Array.isArray(payload) ? payload : [];
    const items = rawItems.map((r) => ({
      id: r && (r.id || r.uuid || r.key || ''),
      name: r && (r.triggerType || r.name || r.title || ''),
      url: r && (r.url || r.html_url || ''),
      status: r && (r.status || r.state || ''),
      created_at: r && (r.lastTriggered || r.created_at || r.createdAt || ''),
      updated_at: r && (r.lastTriggered || r.updated_at || r.updatedAt || ''),
      raw: r
    }));
