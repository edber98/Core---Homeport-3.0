...
module.exports = {
  async activecampaign_geoaddress_list_contact_getgeoipaddress(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/geoIps/{geoAddressID}/geoAddress";
    const geoaddressid = String(d.geoaddressid || '').trim();
    if (!geoaddressid) return { ok: false, error: 'geoaddressid requis.' };
    reqPath = reqPath.replace('{geoaddressid}', encodeURIComponent(geoaddressid));
