async function perplexityRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "https://api.perplexity.ai").replace(/\/+$/, "");
...
module.exports = { utils: { perplexityRequest, parseJsonInput, compactJson, firstTextFromChoices } };
