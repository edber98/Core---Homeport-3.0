...
module.exports = {
  async mailgun_copy_update_put_v3_domain_name_templates_template_name_versions_version(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/v3/{domain_name}/templates/{template_name}/versions/{version_name}/copy/{new_version_name}";
    ...
    const new_version_name = String(d.new_version_name || '').trim();
    if (!new_version_name) return { ok: false, error: 'new_version_name requis.' };
    reqPath = reqPath.replace('{new_version_name}', encodeURIComponent(new_version_name));
