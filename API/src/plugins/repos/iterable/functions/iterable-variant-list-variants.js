...
module.exports = {
  async iterable_variant_list_variants(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/api/experiments/{experimentId}/variants";
    const experimentid = String(d.experimentid || '').trim();
    if (!experimentid) return { ok: false, error: 'experimentid requis.' };
    reqPath = reqPath.replace('{experimentid}', encodeURIComponent(experimentid));
