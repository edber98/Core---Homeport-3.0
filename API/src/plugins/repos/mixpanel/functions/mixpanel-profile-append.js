...
module.exports = {
  async mixpanel_profile_append(node, msg, inputs, opts) {
    const d = inputs || {};
    ...
    const res = await utils.mixpanelIngestRequest(opts, "/engage#profile-append", {
      query: { strict: utils.toInt01(d.strict, false), verbose: utils.toInt01(d.verbose, true) },
      body: { $token: token, $distinct_id: distinctId, $append: values }
    });

    if (!res.ok) return { ok: false, error: res.error, status: res.status, raw: res.details };
    return { ok: true, status: res.status, message: "Valeurs ajoutées au profil.", distinct_id: distinctId, raw: res.data };
  }
};
