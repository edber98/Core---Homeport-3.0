...
module.exports = {
  async mixpanel_group_delete(node, msg, inputs, opts) {
    const d = inputs || {};
    ...
    const res = await utils.mixpanelIngestRequest(opts, "/groups#group-delete", {
      query: { strict: utils.toInt01(d.strict, false), verbose: utils.toInt01(d.verbose, true) },
      body: { $token: token, $group_key: groupKey, $group_id: groupId, $delete: "" }
    });

    if (!res.ok) return { ok: false, error: res.error, status: res.status, raw: res.details };
    return { ok: true, status: res.status, message: "Groupe supprimé.", group_key: groupKey, group_id: groupId, raw: res.data };
  }
};
