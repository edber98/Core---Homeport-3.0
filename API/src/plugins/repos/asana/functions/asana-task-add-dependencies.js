...
module.exports = {
  async asana_task_add_dependencies(node, msg, inputs, opts) {
    const d = inputs || {};
    ...
    const res = await utils.asanaRequest(opts, `/tasks/${encodeURIComponent(taskGid)}/addDependencies`, {
      method: "POST",
      body: { data: { dependencies } }
    });
    if (!res.ok) return { ok: false, error: res.error, status: res.status, details: res.details };
    return { ok: true, status: "success", message: `Dependances ajoutees a la tache ${taskGid}.` };
  }
};
