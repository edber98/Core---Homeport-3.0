...
function cfg(credentials) {
  const c = credentials || {};
  if (!c.host || !c.database || !c.username) return { ok: false, error: "Identifiants PostgreSQL requis: host, database, username." };
...
async function run(key, inputs, opts) {
  ...
  try {
    if (key === "postgresql_query_execute") { const r = await query(credentials, d.sqlQuery, parseJson(d.params, "params", [])); return r.ok ? rowsResult(r) : r; }
    if (key === "postgresql_transaction_execute") { const r = await transaction(credentials, parseJson(d.queries, "queries", [])); return r.ok ? rowsResult(r) : r; }
    if (key === "postgresql_tables_list") { const r = await query(credentials, "SELECT table_schema, table_name, table_type FROM information_schema.tables WHERE table_schema = $1 ORDER BY table_name", [d.schema || "public"]); return r.ok ? { ok: true, tables: r.rows, totalCount: r.rows.length, raw: r } : r; }
    if (key === "postgresql_table_describe") { const r = await query(credentials, "SELECT column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_schema = $1 AND table_name = $2 ORDER BY ordinal_position", [d.schema || "public", d.table]); return r.ok ? { ok: true, columns: r.rows, totalCount: r.rows.length, raw: r } : r; }
    if (key === "postgresql_records_select") {
    ...
    if (key === "postgresql_record_insert") {
    ...
    if (key === "postgresql_records_update") {
      const data = parseJson(d.recordData, "recordData", {});
      const keys = Object.keys(data);
    ...
    if (key === "postgresql_records_delete") {
