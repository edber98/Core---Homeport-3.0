...
module.exports = {
  async apify_task_runs_list(node, msg, inputs, opts) {
    const d = inputs || {};
    ...
    const path = `/actor-tasks/${encodeURIComponent(String(d.taskId))}/runs`;
    const res = await utils.apifyRequest(opts, path, { method: "GET", query });
    if (!res.ok) return { ok: false, error: res.error, status: res.status, details: res.details };
    const rawItems = utils.asArray(res.data?.data || res.data);
    const items = rawItems.map(utils.itemFromUnknown);
    return { ok: true, items, totalCount: Number(res.data?.total || res.data?.count || items.length), nextCursor: "" };
  }
};
