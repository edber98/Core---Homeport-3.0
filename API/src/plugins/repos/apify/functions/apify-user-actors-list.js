...
module.exports = {
  async apify_user_actors_list(node, msg, inputs, opts) {
    ...
    const path = `/acts`;
    const query = ({ limit: d.pageSize, offset: d.cursor });
    const body = {};
    ...
    if (true) {
      const rawItems = utils.asArray(utils.getPath(res.data, "data.items"));
