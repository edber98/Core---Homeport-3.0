...
module.exports = {
  async talkdesk_contact_search_searchcontactbyphone(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/industries/external-entities/contacts/{phone}";
    const phone = String(d.phone || '').trim();
    if (!phone) return { ok: false, error: 'phone requis.' };
    reqPath = reqPath.replace('{phone}', encodeURIComponent(phone));
