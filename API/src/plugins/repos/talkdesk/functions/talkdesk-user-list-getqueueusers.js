...
module.exports = {
  async talkdesk_user_list_getqueueusers(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/ccaas/queues/{queue_id}/users";
    const queue_id = String(d.queue_id || '').trim();
    if (!queue_id) return { ok: false, error: 'queue_id requis.' };
    reqPath = reqPath.replace('{queue_id}', encodeURIComponent(queue_id));
