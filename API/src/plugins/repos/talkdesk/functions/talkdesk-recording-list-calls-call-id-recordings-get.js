...
module.exports = {
  async talkdesk_recording_list_calls_call_id_recordings_get(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/calls/{call_id}/recordings";
    const call_id = String(d.call_id || '').trim();
    if (!call_id) return { ok: false, error: 'call_id requis.' };
    reqPath = reqPath.replace('{call_id}', encodeURIComponent(call_id));
