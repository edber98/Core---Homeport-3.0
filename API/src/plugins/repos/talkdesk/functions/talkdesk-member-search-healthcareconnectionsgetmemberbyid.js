...
module.exports = {
  async talkdesk_member_search_healthcareconnectionsgetmemberbyid(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/healthcare/payers/connections/members/{member_id}";
    const member_id = String(d.member_id || '').trim();
    if (!member_id) return { ok: false, error: 'member_id requis.' };
    reqPath = reqPath.replace('{member_id}', encodeURIComponent(member_id));
