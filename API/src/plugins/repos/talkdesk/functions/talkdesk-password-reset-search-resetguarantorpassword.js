...
module.exports = {
  async talkdesk_password_reset_search_resetguarantorpassword(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/healthcare/guarantors/{guarantor_id}/password-reset";
    const guarantor_id = String(d.guarantor_id || '').trim();
    if (!guarantor_id) return { ok: false, error: 'guarantor_id requis.' };
    reqPath = reqPath.replace('{guarantor_id}', encodeURIComponent(guarantor_id));
