...
module.exports = {
  async talkdesk_entry_search_live_subscriptions_subscription_id_entries_id_put_2(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/live-subscriptions/{subscription_id}/entries/{id}";
    const subscription_id = String(d.subscription_id || '').trim();
    if (!subscription_id) return { ok: false, error: 'subscription_id requis.' };
    reqPath = reqPath.replace('{subscription_id}', encodeURIComponent(subscription_id));
