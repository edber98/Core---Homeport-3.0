...
module.exports = {
  async talkdesk_next_week_opening_hour_list_getlocationnextweekopeninghours(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/locations-directory/locations/{location_id}/next-week-opening-hours";
    const location_id = String(d.location_id || '').trim();
    if (!location_id) return { ok: false, error: 'location_id requis.' };
    reqPath = reqPath.replace('{location_id}', encodeURIComponent(location_id));
