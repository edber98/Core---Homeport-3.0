...
module.exports = {
  async talkdesk_output_list_bulk_imports_users_import_id_outputs_get(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/bulk-imports/users/{import_id}/outputs";
    const import_id = String(d.import_id || '').trim();
    if (!import_id) return { ok: false, error: 'import_id requis.' };
    reqPath = reqPath.replace('{import_id}', encodeURIComponent(import_id));
