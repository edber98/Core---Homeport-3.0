...
module.exports = {
  async wise_transfer_fund(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["profileId","transferId","type"]) {
    ...
    const path = `/v3/profiles/${encodeURIComponent(String(d.profileId))}/transfers/${encodeURIComponent(String(d.transferId))}/payments`;
    const query = {};
    const body = ({ type: String(d.type) });
