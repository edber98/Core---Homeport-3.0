...
module.exports = {
  async wise_recipient_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["profileId","accountHolderName","currency","type","details"]) {
    ...
    const query = {};
    const body = ({ profile: Number(d.profileId), accountHolderName: String(d.accountHolderName), currency: String(d.currency), type: String(d.type), details: json.details });
