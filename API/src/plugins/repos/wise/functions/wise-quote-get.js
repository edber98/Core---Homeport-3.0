...
module.exports = {
  async wise_quote_get(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["profileId","quoteId"]) {
    ...
    const path = `/v3/profiles/${encodeURIComponent(String(d.profileId))}/quotes/${encodeURIComponent(String(d.quoteId))}`;
    const query = {};
    const body = {};
