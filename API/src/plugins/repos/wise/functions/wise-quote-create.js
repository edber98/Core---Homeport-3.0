...
module.exports = {
  async wise_quote_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["profileId","sourceCurrency","targetCurrency","amount","payOut"]) {
    ...
    const path = `/v3/profiles/${encodeURIComponent(String(d.profileId))}/quotes`;
    const query = {};
    const body = ({ sourceCurrency: String(d.sourceCurrency), targetCurrency: String(d.targetCurrency), sourceAmount: Number(d.amount), payOut: String(d.payOut), preferredPayIn: d.preferredPayIn, targetAccount: d.targetAccount ? Number(d.targetAccount) : undefined });
