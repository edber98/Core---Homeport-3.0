...
module.exports = {
  async wise_transfer_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["targetAccount","quoteUuid","customerTransactionId"]) {
    ...
    const query = {};
    const body = ({ targetAccount: Number(d.targetAccount), quoteUuid: String(d.quoteUuid), customerTransactionId: String(d.customerTransactionId), details: json.details || {} });
