...
module.exports = {
  async figma_file_images_get(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["fileKey","ids"]) {
    ...
    const path = `/images/${encodeURIComponent(String(d.fileKey))}`;
    const query = ({ ids: d.ids, format: d.format, scale: d.scale });
    const body = {};
