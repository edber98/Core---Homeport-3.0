...
module.exports = {
  async figma_team_components_list(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["teamId"]) {
    ...
    const path = `/teams/${encodeURIComponent(String(d.teamId))}/components`;
    const query = ({ page_size: d.pageSize, after: d.cursor });
    const body = {};
    ...
    if (true) {
      const rawItems = utils.asArray(utils.getPath(res.data, "meta.components"));
      const items = rawItems.map(utils.itemFromUnknown);
      return { ok: true, items, totalCount: Number(res.data?.pagination?.total || res.data?.total || res.data?.totalCount || items.length), nextCursor: res.data?.meta?.cursor?.after || "" };
    }
