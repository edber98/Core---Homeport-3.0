...
module.exports = {
  async figma_file_comment_create(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["fileKey","message"]) {
      if (d[key] === undefined || d[key] === null || d[key] === "") return { ok: false, error: `${key} requis.` };
    }
    const json = {};
    try {
      for (const key of ["clientMeta"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/files/${encodeURIComponent(String(d.fileKey))}/comments`;
    const query = {};
    const body = ({ message: String(d.message), client_meta: json.clientMeta });
