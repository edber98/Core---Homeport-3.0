...
module.exports = {
  async figma_file_versions_list(node, msg, inputs, opts) {
    ...
    const path = `/files/${encodeURIComponent(String(d.fileKey))}/versions`;
    const query = {};
    const body = {};
    ...
    if (true) {
      const rawItems = utils.asArray(utils.getPath(res.data, "versions"));
