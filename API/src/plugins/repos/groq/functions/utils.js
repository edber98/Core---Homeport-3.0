async function groqRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "https://api.groq.com/openai/v1").replace(/\/+$/, "");
...
module.exports = { utils: { groqRequest, parseJsonInput, compactJson, firstTextFromChoices } };
