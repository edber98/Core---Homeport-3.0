...
module.exports = { async groq_audio_translate(node, msg, inputs, opts) {
  const d = inputs || {};
  ...
  const res = await utils.groqRequest(opts, "/audio/translations", { method: "POST", formData: form });
  if (!res.ok) return { ok: false, error: res.error, status: res.status, details: res.details };
  const text = typeof res.data === "string" ? res.data : res.data?.text || "";
  return { ok: true, id: res.data?.x_groq?.id || "", status: "translated", name: d.model || "whisper-large-v3", url: d.audioUrl, text, result_json: utils.compactJson(res.data) };
} };
