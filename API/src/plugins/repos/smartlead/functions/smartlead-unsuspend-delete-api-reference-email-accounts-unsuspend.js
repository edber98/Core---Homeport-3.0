const { utils } = require('./utils');

module.exports = {
  async smartlead_unsuspend_delete_api_reference_email_accounts_unsuspend(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    let reqPath = "/api/v1/email-accounts/unsuspend/{email_account_id}";
    const email_account_id = String(d.email_account_id || '').trim();
    if (!email_account_id) return { ok: false, error: 'email_account_id requis.' };
    reqPath = reqPath.replace('{email_account_id}', encodeURIComponent(email_account_id));

    const query = {};
    if (d.pageSize !== undefined && d.pageSize !== null && d.pageSize !== '') query.page_size = d.pageSize;
    if (d.page !== undefined && d.page !== null && d.page !== '') query.page = d.page;
    if (d.search !== undefined && d.search !== null && d.search !== '') query.search = d.search;

    const body = undefined;

    log('Requête en cours...');
    const res = await utils.providerRequest(opts, reqPath, { method: 'DELETE', query, body });
    if (!res.ok) return { ok: false, error: res.error, status: res.status, details: res.details };

    return {
      ok: true,
      status: res.status,
      message: 'Action exécutée.',
      raw: res.data || null
    };
  }
};
