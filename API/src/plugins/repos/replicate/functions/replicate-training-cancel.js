...
module.exports = { async replicate_training_cancel(node, msg, inputs, opts) {
  const d = inputs || {};
  if (!d.trainingId) return { ok: false, error: "ID entraînement requis." };
  const res = await utils.replicateRequest(opts, `/trainings/${encodeURIComponent(String(d.trainingId))}/cancel`, { method: "POST" });
