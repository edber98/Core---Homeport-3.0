async function replicateRequest(opts, path, options = {}) {
  const credentials = (opts && opts.credentials) || {};
  const apiKey = credentials.apiKey;
  if (!apiKey) return { ok: false, error: "Clé API manquante." };
  const baseUrl = String(credentials.baseUrl || "https://api.replicate.com/v1").replace(/\/+$/, "");
...
module.exports = { utils: { replicateRequest, parseJsonInput, compactJson, firstTextFromChoices } };
