...
module.exports = {
  async weaviate_object_update(node, msg, inputs, opts) {
    const log = (opts && opts.log) ? opts.log : () => {};
    const d = inputs || {};
    for (const key of ["className","id","properties"]) {
      if (d[key] === undefined || d[key] === null || d[key] === "") return { ok: false, error: `${key} requis.` };
    }
    const json = {};
    try {
      for (const key of ["properties"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/v1/objects/${encodeURIComponent(String(d.className))}/${encodeURIComponent(String(d.id))}`;
    const query = {};
    const body = ({ class: String(d.className), id: String(d.id), properties: json.properties });
    log("Appel API en cours...");
    const options = { method: "PUT", query, body };
    const res = await utils.weaviateRequest(opts, path, options);
