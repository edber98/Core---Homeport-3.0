...
module.exports = {
  async weaviate_objects_batch_create(node, msg, inputs, opts) {
    ...
    const json = {};
    try {
      for (const key of ["objects"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/v1/batch/objects`;
    const query = {};
    const body = ({ objects: json.objects });
