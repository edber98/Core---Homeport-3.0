...
module.exports = {
  async weaviate_graphql_query(node, msg, inputs, opts) {
    ...
    const json = {};
    try {
      for (const key of ["variables"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/v1/graphql`;
    const query = {};
    const body = ({ query: String(d.query), variables: json.variables });
