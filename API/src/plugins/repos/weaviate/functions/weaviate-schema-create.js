...
module.exports = {
  async weaviate_schema_create(node, msg, inputs, opts) {
    ...
    const json = {};
    try {
      for (const key of ["properties","vectorizer"]) json[key] = utils.parseJsonInput(d[key], key, undefined);
    } catch (e) {
      return { ok: false, error: e.message };
    }
    const path = `/v1/schema`;
    const query = {};
    const body = ({ class: String(d.className), properties: json.properties || [], vectorizer: json.vectorizer?.vectorizer || d.vectorizer || undefined, ...((json.vectorizer && typeof json.vectorizer === 'object') ? json.vectorizer : {}) });
