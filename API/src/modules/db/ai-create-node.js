...
module.exports = function(){
  ...
  r.get('/ai/create-node/stream', async (req, res) => {
    try {
      res.status(200);
      ...
      let seedGraph = null;
      try { if (seedB64) { const json = decodeURIComponent(escape(Buffer.from(seedB64, 'base64').toString('utf8'))); seedGraph = JSON.parse(json); } } catch {}
    ...
    } catch (e) { try { res.status(500).json({ success:false, error:'ai_create_node_init_failed', message: String(e?.message || e) }); } catch {} }
  });
