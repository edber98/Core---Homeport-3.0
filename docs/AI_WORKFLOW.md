...
- Backend agent (LangChain tools): `API/src/ai/flow-agent.js`
- Backend SSE endpoint: `API/src/modules/db/ai-flow.js`
- Execution engine (expressions, conditions): `API/src/engine/index.js`
- Expression sandbox: `API/src/engine/expression-sandbox.js`
- Plugins registry/importer: `API/src/plugins/registry.js`, `API/src/plugins/importer.js`, `API/src/plugins/bootstrap.js`
- Plugins manifests (built-in repos + local): `API/src/plugins/repos/*/manifest.json`, `API/src/plugins/local/*/manifest.json`
...
## Form Schema From Flow Agent

- Tool: `ai_generate_form_schema` (backend `API/src/ai/flow-agent.js`).
  - Purpose: Generate a Dynamic Form schema via the AI Form Agent and attach it to a `start_form` node under `model.startFormSchema`.
  - Inputs: `{ nodeId: string; prompt?: string; options?: { layout?: 'vertical'|'horizontal'|'inline'; steps?: boolean; maxFields?: number } }`.
  - Behavior: Calls `runFormAgentWithTools` (AI Form agent) inline, streams events, logs `[ai-form][*]`, collects the final schema, patches the graph, and sends a snapshot.
  - Logs: `[ai-form][call]`, `[ai-form][msg]`, `[ai-form][patch]`, `[ai-form][snapshot]`, `[ai-form][final]`, `[ai-form][attach]`, plus error variants.
  - Side effects: sets `model.startFormEnabled = true` when a schema is attached so the Start Form is active on that node by default.

- System Prompt Update (Flow Agent):
  - If the user intent includes a form (e.g., "formulaire de maintenance"), after `add_node(start_form)`, call `ai_generate_form_schema` to attach the schema, then add follow-up nodes (e.g., `sendmail`) and connect.
  - Example: Start_Form → Generate Form → SendMail.

- Email Integration (sendmail):
  - Use expressions with `payload` (form values) to build `subject`/`body`, e.g., `{{ 'Demande maintenance - ' + (payload?.titre || '') }}`.
  - The Flow Agent avoids hardcoded subjects; it derives subjects from the prompt and/or the form title generated by the AI Form agent.


## Agent Architecture (API/src/ai/flow-agent.js)
...
- Execution (`API/src/engine/index.js`):
...
- Importer: `API/src/plugins/importer.js` enables `expression.allow=true` by default for args fields.
- Manifests:
  - Example condition template (local demo): `API/src/plugins/local/demo/manifest.json`
...
1) Frontend `FlowAiChatComponent` → starts SSE: `GET /api/ai/flow/build/stream?prompt=...` (`API/src/modules/db/ai-flow.js`).
...
- Agent tools: `API/src/ai/flow-agent.js` (search for tool names like `get_node_context_inputs`, `create_node_context`, `connect_by_output_name`).
- Condition items: stabilization logic inside `create_node_context`/`apply_node_params` in the same file.
- Execution sandbox: `API/src/engine/expression-sandbox.js` and usage in `API/src/engine/index.js` (`evaluateCondition`).
- Plugins manifest example: `API/src/plugins/local/demo/manifest.json` (Condition template with args+icon+output_array_field).
